...
class MemberController {
    ...
    public static function index(): void {
        ...
        $stmt = $pdo->prepare("SELECT id, name, email, role, join_date, is_active, created_at FROM members ORDER BY name ASC LIMIT ? OFFSET ?");
    ...
    public static function show(string $id): void {
        AuthMiddleware::requireAnyRole(['admin', 'treasurer', 'board']);
        $pdo = getDbConnection();
        $stmt = $pdo->prepare("SELECT id, name, email, role, join_date, is_active, created_at FROM members WHERE id = ?");
        $stmt->execute([$id]);
        $member = $stmt->fetch();
    ...
    public static function store(): void {
        ...
        $stmt = $pdo->prepare("SELECT id, name, email, role, join_date, is_active FROM members WHERE id = ?");
        $stmt->execute([$id]);
    ...
    public static function update(string $id): void {
        ...
        AuditController::log('members', $id, 'update', $old, $new, AuthMiddleware::getUserId());
    ...
    public static function destroy(string $id): void {
        ...
        AuditController::log('members', $id, 'delete', $old, ['is_active' => 0], AuthMiddleware::getUserId());
